...
import { Stack, useRouter } from 'expo-router'; // Importado Stack para controle local
...
export default function TelaHomeMotorista() {
    ...
    const handleLogout = async () => {
        try {
            ...
            // Apaga a memória de perfil para evitar redirecionamento automático indevido
            await AsyncStorage.removeItem('@user_type'); 
            // Volta para a tela inicial (Tabs)
            router.replace('/(tabs)'); 
        } catch (error) {
    ...
    return (
        <View style={{ flex: 1, backgroundColor: '#000' }}>
            {/* 1. OCULTAR O HEADER DA STACK AQUI */}
            ...
            <ScrollView style={styles.container} contentContainerStyle={styles.scrollContent}>
                <View style={styles.header}>
                    ...
                    {/* Botão de Logout personalizado que substitui a seta */}
...
const styles = StyleSheet.create({
    ...
    btnAcceptText: { fontWeight: 'bold', color: '#000' }
});